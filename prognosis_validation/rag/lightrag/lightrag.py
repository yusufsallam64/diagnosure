...
STORAGES = {
    ...
    "MongoVectorStorage": "atlas_storage",
}
...
class LightRAG:
    ...
    vector_storage: str = field(default="MongoVectorStorage")
    ...
    def __post_init__(self):
        ...
        # Create safe copy of config without unpicklable objects
        global_config = {}
        for k, v in self.__dict__.items():
            if k not in ['_collection', 'collection']:
                try:
                    copy.deepcopy(v)
                    global_config[k] = v
                except:
                    logger.debug(f"Skipping unpicklable config: {k}")
                    continue
        
        if 'working_dir' not in global_config:
            global_config['working_dir'] = self.working_dir  # Add this line
